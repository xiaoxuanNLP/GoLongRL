        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # gen_batch_output has full-sequence tensors (prompt+response);
                    # drop all overlapping keys from new_batch so gen_batch_output wins
                    for key in list(gen_batch_output.batch.keys()):
                        new_batch.batch.pop(key, None)
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        if metric_name == "seq_final_reward":
                            ...
                            # fallback: derive from token_level_scores for terminal-reward setups (e.g. DAPO "score")
