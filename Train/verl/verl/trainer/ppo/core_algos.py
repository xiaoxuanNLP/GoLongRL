...
def compute_grpo_outcome_advantage(
    ...
    difficulty_reweight: bool = False,
    ...
    elif len(token_level_rewards.shape) == 1:
    ...
    with torch.no_grad():
        ...
        if difficulty_reweight:
            # 计算每个 uid 的 pass rate
            # 用 score > uid_mean 定义正确，在 advantage 计算之前的原始 scores 上判断
            # 此时 scores 已经是 normalized advantage 了，sign > 0 等价于原始 score > uid_mean
            id2pass_rate = {}
            for idx in id2score:
                g = len(id2score[idx])
                if g <= 1:
                    # 单个 rollout 无法计算 pass rate，权重设为 1（不调整）
                    id2pass_rate[idx] = 0.5
                else:
                    mean = id2mean[idx]
                    n_correct = sum(1 for s in id2score[idx] if s > mean)
                    id2pass_rate[idx] = n_correct / g

            # 乘上难度权重
            # pass_rate < 0.5 (难题) -> weight > 1 -> 放大 advantage
            # pass_rate > 0.5 (简单题) -> weight < 1 -> 压缩 advantage
            # pass_rate = 0.5 -> weight = 1 -> 不变
            for i in range(bsz):
                # 只对正样本加权，负样本权重保持1
                if scores[i] > 0:
                    pass_rate = id2pass_rate[index[i]]
                    weight = torch.exp(torch.tensor(difficulty_gamma * (0.5 - pass_rate)))
                    weight = torch.clamp(weight, 1.0, 2.0)
...
@register_adv_est(AdvantageEstimator.QWENLONG_GRPO)
def compute_qwenlong_task_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,  # unused, kept for API compatibility
    reward_mode: np.ndarray,
    epsilon: float = 1e-6,
    config: Optional[AlgoConfig] = None,  # unused, kept for API compatibility
    **kwargs,  # unused, kept for API compatibility
) -> tuple[torch.Tensor, torch.Tensor, dict]:
    """Task-aware advantage estimation from QwenLong-L1.5.

    Both mean and std are computed over **all samples of the same task** in the
    current batch (Eq.7 in the paper), instead of per-group (GRPO) or
    full-batch (batch-level) normalization.

    A_i = (r_i - mean_task) / (std_task + epsilon)
    """
    with torch.no_grad():
        scores = token_level_rewards.sum(dim=-1)  # (bs,)

        mode = np.asarray(reward_mode)
        inv_mode = torch.from_numpy(np.unique(mode, return_inverse=True)[1]).to(scores.device)
        num_mode = int(inv_mode.max().item()) + 1

        # ── task-level mean ──────────────────────────────────────────────────
        mode_count = torch.bincount(inv_mode, minlength=num_mode).to(scores.dtype)
        mode_sum = torch.bincount(inv_mode, weights=scores, minlength=num_mode)
        mode_mean = mode_sum / mode_count.clamp_min(1)

        # ── task-level population std ────────────────────────────────────────
        mode_sum2 = torch.bincount(inv_mode, weights=scores * scores, minlength=num_mode)
        mode_var_pop = (mode_sum2 / mode_count.clamp_min(1) - mode_mean * mode_mean).clamp_min(0.0)
        mode_var = mode_var_pop * (mode_count / (mode_count - 1).clamp_min(1))
        mode_std = torch.sqrt(mode_var)

        # tasks with only one sample: std → 1 (keep the centered value as-is)
        single = mode_count <= 1
        mode_std = torch.where(single, torch.ones_like(mode_std), mode_std)

        # ── advantage ────────────────────────────────────────────────────────
        scalars = (scores - mode_mean[inv_mode]) / (mode_std[inv_mode] + epsilon)
...
@register_adv_est(AdvantageEstimator.TMN_GRPO)
def compute_tmn_grpo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    reward_mode: np.ndarray,
    epsilon: float = 1e-6,
    norm_adv_by_task_scale: bool = True,
    difficulty_reweight: bool = False,
    difficulty_gamma: float = 1.0,
    difficulty_smooth_alpha: float = 0.5,  # 0=完全用mode平均, 1=完全用uid自身
    config: Optional[AlgoConfig] = None,
) -> tuple[torch.Tensor, torch.Tensor,dict]:

    with torch.no_grad():
        scores = token_level_rewards.sum(dim=-1)
        uid = np.asarray(index)
        inv_uid = torch.from_numpy(np.unique(uid, return_inverse=True)[1]).to(scores.device)
        num_uid = int(inv_uid.max().item()) + 1

        mode = np.asarray(reward_mode)
        inv_mode = torch.from_numpy(np.unique(mode, return_inverse=True)[1]).to(scores.device)
        num_mode = int(inv_mode.max().item()) + 1

        uid_count = torch.bincount(inv_uid, minlength=num_uid).to(scores.dtype)
        uid_sum = torch.bincount(inv_uid, weights=scores, minlength=num_uid)
        uid_mean = uid_sum / uid_count.clamp_min(1)

        uid_sum2 = torch.bincount(inv_uid, weights=scores * scores, minlength=num_uid)
        uid_mean2 = uid_sum2 / uid_count.clamp_min(1)
        uid_var_pop = (uid_mean2 - uid_mean * uid_mean).clamp_min(0.0)
        uid_var = uid_var_pop * (uid_count / (uid_count - 1).clamp_min(1))

        one = (uid_count <= 1)
        uid_mean = torch.where(one, torch.zeros_like(uid_mean), uid_mean)   # GRPO: mean=0 if n==1
        uid_var = torch.where(one, torch.ones_like(uid_var), uid_var)       # GRPO: std=1 if n==1

        uid_mode = torch.full((num_uid,), -1, device=scores.device, dtype=torch.long)
        uid_mode[inv_uid] = inv_mode
        if (uid_mode < 0).any():
            raise ValueError("TMN-GRPO: found uid with no assigned reward_mode.")

        mode_uid_count = torch.bincount(uid_mode, minlength=num_mode).to(scores.dtype).clamp_min(1)
        mode_var_sum = torch.bincount(uid_mode, weights=uid_var, minlength=num_mode)
        mode_scale = torch.sqrt((mode_var_sum / mode_uid_count).clamp_min(0.0))

        centered = scores - uid_mean[inv_uid]
        if norm_adv_by_task_scale:
            scalars = centered / (mode_scale[inv_mode] + epsilon)
        else:
            scalars = centered
        ...
        extra_metrics = {}
        if difficulty_reweight:
            # 1. 计算任务级别的平均分（同任务内所有 prompt 的 mean 的均值）
            # 语义：这个任务在当前 batch 里整体的分数水平
            mode_mean_sum = torch.bincount(uid_mode, weights=uid_mean, minlength=num_mode)
            mode_mean = mode_mean_sum / mode_uid_count  # shape: (num_mode,)

            # 2. 平滑 uid_mean：混合 uid 自身均值 和 同任务均值
            # alpha=1: 完全用 uid 自身均值（无平滑）
            # alpha=0: 完全用任务均值
            uid_smoothed_mean = (
                difficulty_smooth_alpha * uid_mean
                + (1 - difficulty_smooth_alpha) * mode_mean[uid_mode]
            )
            # 单 rollout 的 uid 直接用任务均值
            uid_smoothed_mean = torch.where(one, mode_mean[uid_mode], uid_smoothed_mean)

            # 3. 用平滑后的 mean 定义正确，计算 pass rate
            is_correct = (scores > uid_smoothed_mean[inv_uid]).float()
            uid_correct = torch.bincount(inv_uid, weights=is_correct, minlength=num_uid)
            uid_pass_rate = uid_correct / uid_count.clamp_min(1)

            # 4. 难度权重
            uid_pos_weight = torch.exp(difficulty_gamma * (0.5 - uid_pass_rate))
            uid_neg_weight = torch.exp(difficulty_gamma * (uid_pass_rate - 0.5))

            extra_metrics["difficulty/mean_pass_rate"] = uid_pass_rate.mean().item()

            # extra_metrics["difficulty/weight_clamp_upper_ratio"] = (uid_weight_raw > 2.0).float().mean().item()
            # extra_metrics["difficulty/weight_clamp_lower_ratio"] = (uid_weight_raw < 1.0).float().mean().item()
            extra_metrics["difficulty/mean_pass_rate"] = uid_pass_rate.mean().item()
            # 5. 对正负样本（score > smoothed_mean）乘权重
            is_positive = scores > uid_mean[inv_uid]
            sample_weight = torch.where(
                is_positive,
                uid_pos_weight[inv_uid],
                uid_neg_weight[inv_uid],
            )
            scalars = scalars * sample_weight
        # ───────────────────────────────────────────────────────────────────
...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str, loss_scale_factor: Optional[int] = None):
    """
